...
########################
    def display_request_popup(request_data: dict) -> None:
...
#######################
    ...
    def create_insert_form():
    
        # Load data into session state if not already present
        session_data = {
            'df_dept': sqlite_db.load_dept_data,
        ...
        with st.container(border=True):
            ...
            department = st.selectbox(
                ...
                options=st.session_state.df_dept['NAME'].tolist(),
                index=None, 
                key="sb_dept")
            dept_code = servant.get_code_from_name(st.session_state.df_dept, department, "CODE")
        ...
        with st.container(border=True):
            ...
            pline = st.selectbox(
                ...
                index=None,
                key="sb_pline")
            pline_code = servant.get_code_from_name(st.session_state.df_pline, pline, "CODE")
        ...
        with st.container(border=True):
            ...
            reqtype = st.selectbox(
                ...
                index=None,
                key="sb_type")
            reqtype_code = servant.get_code_from_name(st.session_state.df_type, reqtype, "CODE")
            ...
            reqcategory = st.selectbox(
                ...
                index=None,
                key="sb_category")
            reqcategory_code = servant.get_code_from_name(st.session_state.df_category, reqcategory, "CODE")
            ...
            detail = st.selectbox(
                ...
                index=None,
                key="sb_detail")
            detail_code = servant.get_code_from_name(st.session_state.df_detail, detail, "CODE")
    ...
    # Main form handling
    request_data = create_insert_form()
    st.divider()
    ...
    if st.button("Submit", type="primary", disabled=save_button_disabled):
        req_nr, rc = sqlite_db.save_request(request_data, conn)
        if rc:
            request_data["reqid"] = req_nr
            if request_data["attachments_list"]:
                rc = sqlite_db.save_attachments(req_nr, request_data["attachments_list"], conn)
            display_request_popup(request_data)
