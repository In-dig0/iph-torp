...
# def show_request_dialog(selected_row_dict, req_status_options, update_request_fn, conn):
#     """Visualizza e gestisci la finestra di dialogo di modifica della richiesta."""
#     popup_title = f'Richiesta {selected_row_dict["REQID"]}'

#     @st.dialog(popup_title, width="large")
#     def dialog_content():
#         reqid = selected_row_dict["REQID"]
#         description = st.session_state.df_requests.loc[st.session_state.df_requests["REQID"] == reqid, "DESCRIPTION"].values[0]
#         st.text_area(label="Descrizione", value=description, disabled=True)

#         st.divider()
#         tdtl_usercode = st.session_state.df_lk_pline_tdtl["USER_CODE"].drop_duplicates().sort_values().tolist()
#         tdtl_username_list = st.session_state.df_users[st.session_state.df_users["CODE"].isin(tdtl_usercode)]["NAME"].tolist()

#         tdtl_default_codes = st.session_state.df_reqassignedto[st.session_state.df_reqassignedto["REQID"] == reqid]["TDTLID"].tolist()
#         default_tdtl_name = st.session_state.df_users[st.session_state.df_users["CODE"].isin(tdtl_default_codes)]["NAME"].tolist()
...
#         idx_status = req_status_options.index(selected_row_dict['STATUS'])
#         req_status = st.selectbox(label=":orange[Status](:red[*])", options=req_status_options, index=idx_status, disabled=False, key="status_selectbox")

#         default_note_td = str(st.session_state.df_requests[st.session_state.df_requests["REQID"] == reqid]["NOTE_TD"].values[0])
#         req_note_td = st.text_area(label=":orange[Tech Department Notes]", value=default_note_td, disabled=False)

#         disable_save_button = (req_note_td == default_note_td) and (selected_row_dict['STATUS'] == req_status) and (req_tdtl_name == default_tdtl_name)
...
# def show_workorder_dialog(selected_row_dict, df_workorders, df_woassignedto, df_users, active_status, default_dept_code, req_status_options, save_workorder_fn, save_woassignments_fn, conn):
#     """Visualizza e gestisci la finestra di dialogo dell'ordine di lavoro."""
#     popup_title = f'Richiesta {selected_row_dict["REQID"]}'
...
#         if "df_user" not in st.session_state or st.session_state.df_user is None:
#             try:
#                 st.session_state.df_user = sqlite_db.load_users_data(conn)
#             except Exception as e:
#                 st.error(f"Errore caricamento dati df_user: {e}")
#                 st.stop()
...
#         st.text_input(label="Product Line", value=selected_row_dict['PRLINE_NAME'], disabled=True)
#         st.text_input(label="Request title", value=selected_row_dict['TITLE'], disabled=True)
#         req_description = st.session_state.df_requests.loc[st.session_state.df_requests["REQID"] == reqid, "DESCRIPTION"]
#         req_description_default = req_description.values[0] if not req_description.empty else ""
#         st.text_input(label="Descrizione della richiesta", value=req_description_default, disabled=True)
...
#         wo_nr = st.session_state.df_workorders[st.session_state.df_workorders["REQID"] == reqid]["WOID"]
#         wo_type_options = ["Standard", "APQP Project"]
#         wo_type_filtered = st.session_state.df_workorders[st.session_state.df_workorders["WOID"] == woid]["TYPE"]
#         wo_type_default = wo_type_filtered.values[0] if not wo_type_filtered.empty else wo_type_options[0]
#         wo_type_index = wo_type_options.index(wo_type_default)

#         wo_startdate_filtered = st.session_state.df_workorders[st.session_state.df_workorders["WOID"] == woid]["STARTDATE"]
#         wo_startdate_default = wo_startdate_filtered.values[0] if not wo_startdate_filtered.empty else None

#         wo_enddate_filtered = st.session_state.df_workorders[st.session_state.df_workorders["WOID"] == woid]["ENDDATE"]
#         wo_enddate_default = wo_enddate_filtered.values[0] if not wo_enddate_filtered.empty else None

#         wo_timeqty_filtered = st.session_state.df_workorders[st.session_state.df_workorders["WOID"] == woid]["TIME_QTY"]
#         wo_timeqty_default = float(wo_timeqty_filtered.iloc[0]) if not wo_timeqty_filtered.empty else 0.0
#         min_value = 0.0
...
#         default_indices = [tdtl_username_list.index(name) for name in default_tdtl_name if name in tdtl_username_list]
#         req_tdtl_name = st.selectbox(
#             label=":orange[Tech Department Team Leader](:red[*])",
#             options=tdtl_username_list,
#             index=default_indices[0] if default_indices else None,
#             key="sb_tdtl_reqmanage2",
#             disabled=False
#         )

#         req_tdtl_code = df_users[df_users["NAME"] == req_tdtl_name]["CODE"].iloc[0] if req_tdtl_name else None

#         wo_type = st.selectbox(label="Type(:red[*])", options=wo_type_options, index=wo_type_index, disabled=False)
#         wo_time_qty = st.number_input(
#             label="Time estimated(:red[*]):",
#             min_value=min_value,
#             value=wo_timeqty_default,
#             step=0.5
#         )
#         wo_time_um = "H"
...
#         wo_assignedto_default_names = []
#         for code in filtered_woassignedto["TDSPID"]:
#             name = servant.get_description_from_code(df_tdusers, code, "NAME")
#             wo_assignedto_default_names.append(name)
...
#         disable_save_button = not (wo_nr.empty or (wo_type == wo_type_default and wo_assignedto == wo_assignedto_default_names and wo_time_qty == wo_timeqty_default))
...
#                 "tdtlid": req_tdtl_code,
#                 "type": wo_type,
#                 "title": selected_row_dict['TITLE'],
#                 "description": req_description_default,
#                 "time_qty": wo_time_qty,
#                 "time_um": wo_time_um,
#                 "status": ACTIVE_STATUS,
#                 "startdate": wo_startdate_default,
#                 "enddate": wo_enddate_default,
#                 "reqid": reqid
#             }
#             wo_idrow, success = sqlite_db.save_workorder(wo, conn)
#             if success:
...
#     with col3:
#         if st.button("📌 Create Work Order", type="secondary", disabled=workorder_button_disable):
#             if selected_rows is not None and not selected_rows.empty:
#                 selected_row_dict = selected_rows.iloc[0].to_dict()
#                 show_workorder_dialog(selected_row_dict, st.session_state.df_workorders, st.session_state.df_woassignedto, st.session_state.df_users, ACTIVE_STATUS, DEFAULT_DEPT_CODE, REQ_STATUS_OPTIONS, sqlite_db.save_workorder, sqlite_db.save_workorder_assignments, conn)
# Python built-in libraries
