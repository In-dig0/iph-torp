...
#            'Status_value',  # Chiave del filtro status nella sidebar
    ...
    def show_request_dialog(selected_row, req_status_options, update_request_fn):
        ...
        def dialog_content():
            ...
            selected_pline_code = st.sessione_state.df_pline[df_pline["NAME"] == selected_pline_name]["CODE"].values[0]
            ...
            # Display Description
            description = st.sessione_state.df_requests[st.sessione_state.df_requests["REQID"] == reqid]["DESCRIPTION"].values[0]
            st.text_area(label="Description", value=description, disabled=True)

            st.divider()
            tdtl_usercode = st.sessione_state.df_lk_pline_tdtl["USER_CODE"].drop_duplicates().sort_values().tolist() #conversione in lista
            tdtl_username_list = st.sessione_state.df_users[st.sessione_state.df_users["CODE"].isin(tdtl_usercode)]["NAME"].tolist()

            tdtl_default_codes = st.sessione_state.df_reqassignedto[st.sessione_state.df_reqassignedto["REQID"] == reqid]["USERID"].tolist()

            if tdtl_default_codes:
                tdtl_option = st.sessione_state.df_users[st.sessione_state.df_users["CODE"].isin(tdtl_default_codes)]
            ...
            if req_tdtl_name:
                req_tdtl_code = st.sessione_state.df_users[st.sessione_state.df_users["NAME"].isin(req_tdtl_name)]["CODE"].tolist()
            else:
                req_tdtl_code = []
            ...
            default_note_td = str(st.sessione_state.df_requests[st.sessione_state.df_requests["REQID"] == reqid]["NOTE_TD"].values[0])
...
#######################################
    def show_workorder_dialog(selected_row, df_workorders, df_woassignedto, df_users, active_status, default_dept_code, req_status_options, save_workorder_fn, save_woassignments_fn):
        ...
        def dialog_content():
            ...
            req_description = st.sessione_state.df_requests[st.sessione_state.df_requests["REQID"]==reqid]["DESCRIPTION"]
            ...
            req_note_td = st.sessione_state.df_requests[st.sessione_state.df_requests["REQID"]==reqid]["NOTE_TD"]
            ...
            wo_nr = st.sessione_state.df_workorders[st.sessione_state.df_workorders["REQID"] == reqid]["WOID"]

            wo_type_options=["Standard", "APQP Project"]  #APQP -> ADVANCED PRODUCT QUALITY PLANNING"  
            wo_type_filtered = st.sessione_state.df_workorders[st.sessione_state.df_workorders["WOID"] == woid]["TYPE"]
            ...
            wo_startdate_filtered = st.sessione_state.df_workorders[st.sessione_state.df_workorders["WOID"] == woid]["STARTDATE"]
            ...
            wo_enddate_filtered = st.sessione_state.df_workorders[st.sessione_state.df_workorders["WOID"] == woid]["ENDDATE"]
            ...
            wo_timeqty_filtered = st.sessione_state.df_workorders[st.sessione_state.df_workorders["WOID"] == woid]["TIME_QTY"]
